...
class SealedDES
{
	...
	protected Cipher des_cipher;
	
	// Key for the class
	protected SecretKeySpec the_key = null;

	// Byte arrays that hold key block
	protected byte[] deskeyIN = new byte[8];
	protected byte[] deskeyOUT = new byte[8];
	...
	//
	protected void makeDESKey(byte[] in, byte[] out)  
	{
    ...
    maxkey = maxkey >>> (64 - keybits);
		...
		for ( long i = 0; i < maxkey; i++ )
		{
			...
			temp = sldObj;
			deccipher.setKey ( i );
			String decryptstr = deccipher.decrypt ( temp );
			...
			if (( decryptstr != null ) && ( decryptstr.indexOf ( "Hopkins" ) != -1 ))
			{
				...
				p.printf("Found decrypt key %d producing message: %s\n", i , decryptstr);
